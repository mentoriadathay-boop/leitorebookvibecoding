...
const phaseColors = [
  'border-l-[#5B2A6E] bg-[#F2E4FA]/40 dark:bg-[#5B2A6E]/10',
  'border-l-[#C2298A] bg-[#FCE4F1]/40 dark:bg-[#C2298A]/10',
  'border-l-[#F5B942] bg-stargold-50/40 dark:bg-stargold-900/10',
  'border-l-coral-500 bg-coral-50/40 dark:bg-coral-900/10',
    ...
    <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input type="number" min="1" max="60" value={form.tempo_disponivel} onChange={e => set('tempo_disponivel', e.target.value)}
                className="w-20 text-xs border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:border-[#5B2A6E] bg-white dark:bg-[#111] text-gray-700 dark:text-gray-300" />
      ...
      {result && (
        <div className="space-y-4">
          <div className="flex items-center gap-3 px-4 py-3 bg-[#3E1B4D] text-white rounded-xl">
            ...
            <div key={i} className={`border-l-4 rounded-xl p-5 ${phaseColors[i % phaseColors.length]}`}>
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {fase.indicadores?.map((ind, j) => <li key={j} className="text-xs text-[#5B2A6E] dark:text-magic-light">✓ {ind}</li>)}
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {result.metas_semanais?.map((m, i) => (
                <div key={i} className="flex items-start gap-2 text-xs text-gray-700 dark:text-gray-300">
                  <span className="text-[#F5B942] shrink-0">→</span>{m}
                </div>
              ))}
            </div>
          </div>
          ...
          {result.conteudos_hub_relacionados?.length > 0 && (
            <div className="bg-[#F2E4FA] dark:bg-[#3E1B4D]/20 rounded-xl p-4">
              <p className="text-xs font-bold text-[#5B2A6E] dark:text-magic-light mb-2">📚 Conteúdos do Hub relacionados</p>
              <ul className="space-y-1">
